...
window.addEventListener('load', function () {
    ...
    function getPrice() {
        ...
        return price.toFixed(2);
>>>>>>> 2072d9136419b2f18e9d54113a26eda0632bedc7
    }
...
>>>>>>> 2072d9136419b2f18e9d54113a26eda0632bedc7
        ...
        for (let i = 0; i < ExistChartOrder.length; i++) {
          ...
          if (currentuser.length == 0) {
            imge.src =
              "../images/flowers/" +
              getProductImgByIdguest(ExistChartOrder[i].productId);
          } else {
            imge.src =
              "../images/flowers/" +
          ...
          Totalprice.appendChild(
            document.createTextNode(getPriceForProduct(i))
          );
...
<<<<<<< HEAD
    function getProductImgById(id) {
      ...
      let resultOrdeId = TotalOrders.find(
        (order) =>
          order.productId === parseInt(id) && order.user == currentuser.id
      );
      let searchedProduct =
        JSON.parse(localStorage.getItem("flowersData")) || [];

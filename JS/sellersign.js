...
window.addEventListener("load", function () {
  ...
  save_sign.addEventListener('click', function () {
    ...
    if (!selAutho.valid_name(shop_name)) {
      ...
      selAutho.handleTheErrorMessage(2, "This email is already registered. Please use a different email.");
    }
    else if (email.trim() == 0 || !selAutho.isValidEmail(email)) {
      selAutho.handleTheErrorMessage(2, "This email is not a valid email,please try another one.");
    }
    else if (!selAutho.password_valid(password)) {
      selAutho.handleTheErrorMessage(3, "This is an invalid password. Please make sure it is at least 8 characters long and choose a complex one.");
    }
    else if (!selAutho.iscomplexPassword(password)) {
      selAutho.handleTheErrorMessage(3, "This is not a complex password. Please choose a password with at least 8 characters, including letters, digits, and special characters.");
    }
    else if (number_prod.trim() == 0 || !selAutho.check_positive(number_prod)) {
      selAutho.handleTheErrorMessage(4, "please enter a valid number of your products");
    }
